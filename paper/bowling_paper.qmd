...
3. **Markov chain model with spare difficulty.** Strike probability follows a first-order Markov chain with state-dependent transition probabilities calibrated from empirical ratios reported by @yaari2012: $P(\text{strike} \mid \text{prev. strike}) = p_s \times 1.15$, $P(\text{strike} \mid \text{prev. spare}) = p_s \times 0.95$, $P(\text{strike} \mid \text{prev. open}) = p_s \times 0.85$. Spare conversion is leave-dependent, with three difficulty classes: easy single-pin leaves (55% of non-strike results, high conversion), moderate multi-pin leaves (35%, baseline conversion), and splits (10%, conversion rate reduced to 30% of baseline). This approach follows the Markov modelling framework of @vanderwerken2018.
