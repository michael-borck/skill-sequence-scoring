...
## The Independence Assumption and Momentum

Our base simulation assumes frame-to-frame independence: each ball's outcome depends only on the player's fixed skill parameters, not on preceding results. This is a conservative assumption. The hot hand literature in bowling [@yaari2012; @dorseypalmateer2004] provides empirical evidence that consecutive strikes are positively correlated — a "momentum" or streakiness effect.

To test whether this matters, we ran a supplementary simulation with a momentum model in which strike probability increases by 5 percentage points after a strike and decreases after an open frame. The result (@fig-momentum) is that momentum *amplifies* the traditional scoring advantage at every tier above competitive level. This is expected: when streaks are more likely, the compounding bonus structure of traditional scoring has more opportunities to generate score separation. The independence model therefore provides a *lower bound* on the traditional scoring advantage — the real-world effect is likely larger.
...
This paper has several limitations that suggest directions for future work. First, the simulation uses a parametric skill model rather than real game data. Validating the results against a database of actual PBA tournament scores — applying both scoring systems to the same ball sequences — would provide a powerful empirical anchor. Second, while we model a simple momentum effect, more sophisticated models incorporating fatigue, lane condition changes, and frame-specific strategy could refine the magnitude estimates. Third, our analysis focuses on individual game scores; extending to multi-game series and tournament formats would address whether the discrimination differences compound over longer competitions. The source code accompanying this paper is designed to support these extensions.
